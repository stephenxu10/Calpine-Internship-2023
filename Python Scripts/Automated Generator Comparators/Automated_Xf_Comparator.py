...
with zipfile.ZipFile(zip_data, 'r') as zip_file:
    ...
    with zipfile.ZipFile(today_data, 'r') as first_zip_file:
        csv_now = [x for x in first_zip_file.namelist() if xf_key in x][0]
    ...
    with zipfile.ZipFile(tomo_data, 'r') as second_zip_file:
        csv_tomorrow = [x for x in second_zip_file.namelist() if xf_key in x][0]
...
if len(change) == 0:
    body += '<html><p> No transformer statuses have changed from yesterday to today. <br> </p></html>'

body += '<html><body>' + df_status_comp.to_html(index=False) + '</body></html>'

msg = MIMEMultipart('alternative')
msg['Subject'] = "Daily DAM Transformer Comparison Results"
